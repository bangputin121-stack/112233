...
async def adm_panel_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    # Clear pending state biar nggak ada aksi nyangkut (broadcast, set_coins, dll)
    ctx.user_data.pop("adm_action", None)
    ctx.user_data.pop("adm_target_id", None)
    ctx.user_data.pop("adm_pending_broadcast", None)
...
async def adm_text_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "set_welcome":
        ...
        # Step 1: simpen pesan, tampilin preview + konfirmasi
        # JANGAN langsung kirim, butuh konfirmasi dulu biar nggak miss-send
        msg_text = text
        if not msg_text or len(msg_text.strip()) < 1:
            await update.message.reply_text("❌ Pesan kosong, broadcast dibatalkan.")
            ctx.user_data.pop("adm_action", None)
            return

        # Simpen pesan ke pending state
        ctx.user_data["adm_pending_broadcast"] = msg_text
        ctx.user_data.pop("adm_action", None)  # clear state biar nggak nyangkut

        # Hitung total user buat preview
        async with get_db() as db:
            total = (await fetchone(db, "SELECT COUNT(*) as c FROM users"))["c"]
        ...
        preview = (
            ...
            f"{msg_text}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 Bakal dikirim ke: **{total} player**\n\n"
            f"⚠️ Yakin mau kirim?"
        )
        await update.message.reply_text(
            preview,
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Ya, Kirim", callback_data="adm_broadcast_confirm"),
                    InlineKeyboardButton("❌ Batal", callback_data="adm_broadcast_cancel"),
                ]
            ]),
            parse_mode=ParseMode.MARKDOWN
        )
...
@admin_only
async def adm_broadcast_confirm_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Eksekusi broadcast setelah konfirmasi."""
    query = update.callback_query
    await query.answer()
    msg_text = ctx.user_data.get("adm_pending_broadcast")
    if not msg_text:
        await query.edit_message_text("❌ Tidak ada pesan pending. Mungkin udah expired.")
        return

    # Hapus state IMMEDIATELY biar nggak bisa di-double click
    ctx.user_data.pop("adm_pending_broadcast", None)
    ctx.user_data.pop("adm_action", None)

    await query.edit_message_text("⏳ Lagi ngirim broadcast, tunggu sebentar...")
    ...
    await query.edit_message_text(
        ...
        f"📤 Terkirim: **{sent}**\n"
        f"❌ Gagal: **{failed}**\n"
        f"📊 Total: **{sent + failed}** player",
        parse_mode=ParseMode.MARKDOWN
    )


@admin_only
async def adm_broadcast_cancel_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Batalin broadcast yang lagi pending."""
    query = update.callback_query
    await query.answer("Broadcast dibatalkan", show_alert=True)
    # Hapus state
    ctx.user_data.pop("adm_pending_broadcast", None)
    ctx.user_data.pop("adm_action", None)
    await query.edit_message_text(
        "❌ **Broadcast dibatalkan**\n\nPesan TIDAK dikirim ke siapapun.",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("⬅️ Kembali ke Panel", callback_data="adm_panel")
        ]]),
        parse_mode=ParseMode.MARKDOWN
    )
...
async def event_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    Bikin event baru dengan multiplier coin/xp.
    Format: /event <coin_mult> <xp_mult> <duration_jam> <nama event> | <deskripsi>

    Contoh:
    /event 2 2 24 Weekend Bonus | Coin & XP x2 selama 24 jam!
    /event 3 1.5 6 Flash Sale | Coin x3, XP x1.5 selama 6 jam
    """
    ...
    if not args_text:
        await update.message.reply_text(
            ...
            "`/event <coin_mult> <xp_mult> <jam> <nama> | <deskripsi>`\n\n"
            "Contoh:\n"
            "`/event 2 2 24 Weekend Bonus | Coin & XP 2x selama 24 jam!`\n"
            "`/event 3 1.5 6 Flash Sale | Coin 3x, XP 1.5x 6 jam`\n\n"
            "**Aturan:**\n"
            "• Multiplier min 1.0 (1.0=normal, 2.0=2x, dst)\n"
            "• Durasi 1-168 jam (max 7 hari)\n"
            "• Pake `|` buat pisahin nama & deskripsi\n"
            "• Multiple event bakal DIKALIIN (2 event x2 = total x4)",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    try:
        ...
        first = parts[0].strip().split(None, 3)
        if len(first) < 4:
            raise ValueError("Format salah")
        coin_mult = float(first[0])
        xp_mult = float(first[1])
        duration = int(first[2])
        name = first[3].strip()
        description = parts[1].strip() if len(parts) > 1 else ""
    except (ValueError, IndexError):
        await update.message.reply_text(
            "❌ Format salah.\n"
            "Contoh: `/event 2 2 24 Weekend Bonus | Coin & XP 2x`",
            parse_mode=ParseMode.MARKDOWN
        )
        return

    from game.engine import admin_create_event
    ok, msg, event_data = await admin_create_event(
        name, description, coin_mult, xp_mult, duration, update.effective_user.id
    )
    await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
    ...
    sent = 0
    broadcast_text = (
        f"🎉 **EVENT BARU DIMULAI!**\n\n"
        f"🌟 **{name}**\n"
        f"📝 {description}\n\n"
        f"💰 Coin Multiplier: **{coin_mult}x**\n"
        f"⭐ XP Multiplier: **{xp_mult}x**\n"
        f"⏰ Durasi: **{duration} jam**\n\n"
        f"Buruan main sekarang, manfaatin event-nya! 🚀"
    )
    for u in users:
        try:
            await ctx.bot.send_message(
                u["user_id"], broadcast_text, parse_mode=ParseMode.MARKDOWN
            )
            sent += 1
        except Exception:
            pass
...
@admin_only
async def stopevent_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Hentikan event secara manual. Format: /stopevent <event_id>"""
    args = ctx.args
    if len(args) < 1:
        await update.message.reply_text(
            "📛 **Stop Event**\n\n"
            "Format: `/stopevent <event_id>`\n\n"
            "Cek event_id dengan `/listevents` dulu.",
            parse_mode=ParseMode.MARKDOWN
        )
        return
...
@admin_only
async def listevents_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """List semua event aktif."""
    from game.engine import get_active_events
    events = await get_active_events()

    if not events:
        await update.message.reply_text("📭 Tidak ada event aktif saat ini.")
        return
    ...
    lines = ["🎉 **EVENT AKTIF:**\n"]
    for e in events:
        ends_at = e.get("_ends_dt")
        if ends_at:
            remaining = int((ends_at - now).total_seconds())
            hours = remaining // 3600
            minutes = (remaining % 3600) // 60
            time_left = f"{hours}j {minutes}m"
        else:
            time_left = "?"
        lines.append(
            f"🪧 **ID {e['id']}** — {e['name']}\n"
