...
async def sell_menu_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    price_line = ""
    if sell_price:
        # Tampilin harga asli + harga setelah event (kalau ada)
        from game.engine import get_event_multipliers
        ...
        if double == "1":
            actual_price *= 2
        if coin_mult > 1.0:
            actual_price = int(actual_price * coin_mult)
        if actual_price != sell_price:
            price_line = f"💵 Harga jual: ~~Rp{sell_price:,}~~ → **Rp{actual_price:,}**/satuan 🎉"
        else:
            price_line = f"💵 Harga jual: Rp{sell_price:,}/satuan"
    else:
        price_line = "⚠️ Tidak bisa dijual langsung (pasang di pasar saja)"
...
async def fixme_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Reset stuck state untuk player. Hapus semua pending action & state."""
    user = update.effective_user
    # Clear all user_data state
    ctx.user_data.clear()

    # Reset any stuck admin actions
    ctx.user_data.pop("adm_action", None)
    ctx.user_data.pop("adm_target_id", None)
    ctx.user_data.pop("adm_pending_broadcast", None)
    ctx.user_data.pop("listing_item", None)
    ctx.user_data.pop("listing_qty", None)
    ctx.user_data.pop("farm_page", None)
    ctx.user_data.pop("pens_page", None)

    await safe_send(update, (
        "🔧 **AKUN DIRESET!**\n\n"
        "✅ State internal dibersihkan\n"
        "✅ Pending action dihapus\n\n"
